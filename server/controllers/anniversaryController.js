...
export const runSync = async (req, res, { draft = false } = {}) => {
    try {
        ...
        let avisoPatron = null;
        {
            ...
            // pagadas por pieza.
            //
            // ⚠️ Y DESDE v4.1063 COMPARTEN EL REINTENTO CON LA TERCERA: la
            // ORTOGRÁFICA. Es la única que NO cuelga de `styleGuard`, y la
            // asimetría es deliberada: aquél es un criterio de dirección de
            // arte que un administrador puede querer apagar; el nombre de un
            // club escrito con su tilde no es una preferencia estética, es el
            // nombre de una organización. Sumarla al MISMO array es lo que
            // mantiene el presupuesto en una generación de reintento por pieza
            // —tres puertas con tres reintentos serían cuatro generaciones
            // pagadas— y hace que los avisos salgan juntos cuando coinciden.
            const veredictos = [];
                ...
                if (reclamo) {
                    try {
                        ...
                        const otra = await startComposition({
                            ...
                            clubName: piece.clubName, years: piece.years,
                            seed: piece.id,
                            extraClause: duros.map(x => x.clause).join(' '),
                        ...
                        return res.json({ status: 'composing', ready: false, retrying: true, reason: `${duros.map(x => x.v.note).join(' ')} Se está regenerando con la instrucción reforzada.` });
                    } catch (e2) {
                        console.warn('[anniversary] el reintento del patrón no pudo despachar:', e2.message);
                    }
                }
            }
            if (duros.length) avisoPatron = `${duros.map(x => x.v.note).join(' ')} Se entrega igual: mirala y, si no sirve, «Volver a probar».`;
        }
