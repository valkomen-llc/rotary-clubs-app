...
export const generatePost = async (req, res) => {
    try {
        ...
        console.log(`[STUDIO] Requesting Resilient Regeneration (${dalleSize})...`);
        
        let finalUrl = null;
        let usedModel = "dall-e-3";

        // PASS 1: Attempt DALL-E 3 HD
        try {
            const dalleResponse = await fetch('https://api.openai.com/v1/images/generations', {
                ...
                body: JSON.stringify({
                    model: "dall-e-3",
                    prompt: `Professional photography for Rotary. CINEMATIC OUTPAINTING. ${parsed.visual_prompt.substring(0, 800)}. Recreate context.`,
                    n: 1,
                    size: dalleSize,
                    quality: "hd"
                })
        ...
        // PASS 2: Emergency Fallback to DALL-E 2
        if (!finalUrl) {
            try {
                console.log('[STUDIO] Triggering DALL-E 2 Fallback...');
                const d2Response = await fetch('https://api.openai.com/v1/images/generations', {
                    ...
                    body: JSON.stringify({
                        model: "dall-e-2",
                        prompt: `Rotary institutional scene. ${parsed.visual_prompt.substring(0, 400)}`,
                        n: 1,
                        size: "1024x1024"
                    })
                });
                const d2Data = await d2Response.json();
                if (d2Data.data?.[0]?.url) {
                    finalUrl = d2Data.data[0].url;
                    usedModel = "dall-e-2";
                }
            } catch (e) { console.error('[STUDIO] DALL-E 2 failed'); }
        }

        // PASS 3: SALVAVIDAS (Original Image)
        // If everything fails, we use the original image so the user is never blocked
        if (!finalUrl) {
            console.log('[STUDIO] AI Saturated. Preserving original image for continuity.');
            finalUrl = imageUrl;
            usedModel = "original-preserved";
        }
        ...
        res.json({
            ...
            metadata: { 
                engine: usedModel, 
                quality: finalUrl === imageUrl ? 'original' : (usedModel === 'dall-e-3' ? 'HD' : 'SD'),
                format: finalUrl === imageUrl ? 'original' : (isLandscape ? '16:9' : '9:16')
            }
        });
