...
console.log('[emailAutomationController] v4.442 — secuencias automáticas por etiqueta');
...
const normalizeSteps = (steps) => (Array.isArray(steps) ? steps : [])
    .filter((s) => s && s.subject && s.content)
    .map((s, i) => ({
        order: i,
...
export const processEmailAutomations = async ({ baseUrl, now = new Date() } = {}) => {
    ...
    for (const enrollment of due) {
        ...
        try {
            ...
            console.error(`[emailAutomation] envío de paso falló (enrollment ${enrollment.id}):`, error.message);
        }
