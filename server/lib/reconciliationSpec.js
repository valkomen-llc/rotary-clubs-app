...
export const describeTransferScope = ({ elegidos = 0, cubiertos = 0, lotes = 1, sueltos = 0 } = {}) => {
    const partes = [];
    if (lotes > 1) {
        partes.push(`Los ${elegidos} aportes elegidos pertenecen a ${lotes} traslados distintos: se envía una conciliación por cada uno.`);
    }
    if (cubiertos > elegidos) {
    ...
    if (sueltos > 0) {
        partes.push(`${sueltos} aporte${sueltos === 1 ? '' : 's'} se registró como traslado suelto, sin grupo: no tiene una conciliación de traslado que reenviar.`);
    }
    return partes;
};
 ...
 */
export const validateResend = ({ batch = null, items = [], recipients = null } = {}) => {
    const errores = [];
    const avisos = [];
    ...
    if (batch?.id && !vivos.length) {
        errores.push('Todos los aportes de este traslado están reversados: la conciliación quedaría vacía.');
    }
    const reversados = (Array.isArray(items) ? items : []).length - vivos.length;
    if (reversados > 0) {
        avisos.push(`${reversados} aporte${reversados === 1 ? '' : 's'} de este traslado está reversado y no entra en la conciliación.`);
    }
...
export default {
    ...
    validateResend,
    RECONCILIATION_COLUMNS, processorFeeOf, reconciliationTotals, toWinAnsi,
