...
import { Loader2, Plus, Trash2, Save, Clock, CalendarOff, UserCog, Settings2, Link2, Video } from 'lucide-react';
...
const AvailabilityConfig: React.FC = () => {
    ...
    const updResponsibleLink = async (id: string, url: string) => {
        const res = await fetch(`${API}/training/admin/responsibles/${id}`, { method: 'PUT', headers: H, body: JSON.stringify({ provisionalMeetingUrl: url }) });
        if (res.ok) { setResponsibles(rs => rs.map(x => x.id === id ? { ...x, provisionalMeetingUrl: url } : x)); toast.success('Enlace del facilitador guardado'); }
        else { const d = await res.json().catch(() => ({})); toast.error(d.error || 'No se pudo guardar el enlace'); }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Enlace provisional de reunión */}
            <section className="bg-white rounded-2xl border border-gray-100 p-5">
                <h3 className="font-black text-gray-900 flex items-center gap-2 mb-1"><Video className="w-5 h-5 text-indigo-600" />Enlace provisional de reunión</h3>
                <p className="text-xs text-gray-500 mb-4">Este enlace se utilizará en las reservas mientras se completa la integración automática con Google Meet o Zoom.</p>
                <label className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-3">
                    <input type="checkbox" checked={!!cfg.provisionalMeetingEnabled} onChange={e => setCfg((c: any) => ({ ...c, provisionalMeetingEnabled: e.target.checked }))} />
                    Usar enlace provisional en las nuevas reservas
                </label>
                <div className="relative">
                    <Link2 className="w-4 h-4 text-gray-400 absolute left-3 top-3" />
                    <input value={cfg.provisionalMeetingUrl || ''} onChange={e => setCfg((c: any) => ({ ...c, provisionalMeetingUrl: e.target.value }))}
                        placeholder="https://meet.google.com/xxx-xxxx-xxx o https://zoom.us/j/..." className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-gray-200 text-sm" />
                </div>
                {cfg.provisionalMeetingUrl && !/^https:\/\/\S+$/i.test(cfg.provisionalMeetingUrl.trim()) && (
                    <p className="text-xs text-red-500 mt-1">El enlace debe iniciar con https://</p>
                )}
                <p className="text-[11px] text-gray-400 mt-3">Prioridad del enlace: <b>facilitador</b> → <b>tipo de cita</b> → <b>global</b> (este). Cuando se active Zoom/Meet automático, ese enlace tendrá prioridad sobre el provisional.</p>
                <button onClick={saveConfig} disabled={saving} className="mt-4 inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-indigo-600 text-white font-bold text-sm disabled:opacity-60">
                    {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}Guardar enlace
                </button>
            </section>
                            ...
                            {responsibles.map(r => (
                                <div key={r.id} className="flex flex-wrap items-center gap-2 text-sm">
                                    ...
                                    <div className="relative flex-1 min-w-[160px]">
                                        ...
                                        <input defaultValue={r.provisionalMeetingUrl || ''} placeholder="Enlace provisional del facilitador (opcional)"
                                            onBlur={e => updResponsibleLink(r.id, e.target.value)}
                                            className="w-full pl-8 pr-2 py-1.5 rounded-lg border border-gray-200 text-xs" />
                                    </div>
