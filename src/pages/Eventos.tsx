...
const Eventos = () => {
    ...
    // Mock events for the premium look — in the real app, these would come from an API
    const events = [
        {
            ...
            <section className="bg-white border-b border-gray-100 sticky top-16 z-40">
                <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex gap-8">
                        {['Todos', 'Servicio', 'Integración', 'Institucional'].map((cat, i) => (
                            <button
                                key={cat}
                                className={`text-sm font-bold uppercase tracking-widest ${i === 0 ? 'text-rotary-blue' : 'text-gray-400 hover:text-gray-600'}`}
                            >
                                {cat}
                            </button>
                        ))}
