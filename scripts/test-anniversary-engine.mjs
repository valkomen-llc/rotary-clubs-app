...
// v4.907 quitó las puertas; v4.910 devolvió EXACTAMENTE UNA por directiva
// del cliente: el patrón visual del fondo (judgeStylePattern), con una sola
// regeneración y entrega-con-aviso. Las demás (franja, texto dibujado,
// preservación) siguen fuera del flujo — verifyComposition es sólo del
// benchmark. Si reaparece en el sondeo, esto falla.
check('v4.910: la única puerta del sondeo es el patrón visual',
    (() => { const s = ctrl.slice(ctrl.indexOf('export const runSync'), ctrl.indexOf('export const pieceView'));
        return !/verifyComposition\(/.test(s) && /judgeStylePattern\(/.test(s) && /STYLE_RETRY_CLAUSE/.test(s); })());
